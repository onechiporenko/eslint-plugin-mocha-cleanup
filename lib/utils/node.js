...
function isParentSinonAssert(node) {
  return obj.get(node, "parent.object.object.name") === "sinon" &&
    (obj.get(node, "parent.object.property.name") === "assert" || obj.get(node, "parent.object.property.value") === "assert");
}
...
function isParentChaiShould(node) {
  return (node.name === "should" || node.value === "should") &&
    obj.get(node, "parent.type") === "MemberExpression" &&
    (obj.get(node, "parent.property.name") === "should" || obj.get(node, "parent.property.value") === "should");
}
...
function isParentChaiExpect(node) {
  return node.name === "expect" &&
    ...
    obj.get(node, "parent.callee.name") === "expect";
}
...
function isParentChaiAssert(node) {
  return node.name === "assert" &&
    (
      obj.get(node, "parent.type") === "MemberExpression" &&
      obj.get(node, "parent.object.name") === "assert"
        ||
      obj.get(node, "parent.type") === "CallExpression" &&
      obj.get(node, "parent.callee.name") === "assert"
    );
}
...
function isChaiAssert(node) {
  return (
      ...
      obj.get(node, "object.name") === "assert"
    )
      ||
    (
      node.type === "CallExpression" &&
      obj.get(node, "callee.name") === "assert"
    );
}
...
function getParentExpression(node) {
  ...
  return node.parent ? getParentExpression(node.parent) : null;
}
