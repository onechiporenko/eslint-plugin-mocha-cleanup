...
function isChaiChainable(node) {
  return (
    node.type === 'MemberExpression' && chainable.includes(node.property.name)
  );
}
...
function _isChaiAssert4(node) {
  return (
    ...
    (node.object.property.name === 'assert' ||
      node.object.property.value === 'assert')
  );
}
...
function isBlockBody(node, type) {
  ...
  const parentType = obj.get(parent, 'callee.object')
    ? 'MemberExpression'
    : 'Identifier';
  ...
  if (parentType === 'Identifier') {
    return (
      types.includes(obj.get(parent, 'callee.name')) &&
      (obj.get(parent, 'arguments.0') === node ||
        obj.get(parent, 'arguments.1') === node)
    );
  }
 ...
 * @returns {?ASTNode}
 */
function getParentCallOrMemberExpression(node) {
  return _getParentByTypes2(node, ['MemberExpression', 'CallExpression']);
}
 ...
 * @private
 */
function _getParentByTypes2(node, types) {
  if (!node.parent) {
    return null;
  }
  const _types = Array.isArray(types) ? types : [types];
  if (_types.includes(node.parent.type)) {
    return _getParentByTypes2(node.parent, _types);
  } else {
    return node;
  }
}
...
module.exports = {
  ...
  getParentExpression,
  getParentCallOrMemberExpression,
};
