...
module.exports = function (context) {
  ...
  var ignoreZeroAssertionsIfDoneExists = context.options.length > 2 ? context.options[2] : true;
  var nodeSkipped;
  var doneExists;
  ...
  return {
    ...
    "FunctionExpression:exit": function (node) {
      if (n.isTestBody(node)) {
        ...
        if (assertCounter === 0 && !nodeSkipped && !(ignoreZeroAssertionsIfDoneExists && doneExists)) {
          context.report(node.parent, "`it` without assertions is not allowed.");
        }
        insideIt = false;
        nodeSkipped = false;
        doneExists = false;
