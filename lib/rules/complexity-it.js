...
module.exports = function (context) {
  ...
  return {
    ...
    "FunctionExpression:exit": function (node) {
      if (n.isItBody(node)) {
        if (currentComplexityCount > maxAllowedComplexity) {
          context.report(node.parent, "`it` has a complexity of {{num}}. Maximum allowed is {{max}}.", {
    ...
    "MemberExpression": function (node) {
      if (n.isSinonAssert(node) || n.isAssert(node) || n.isShould(node) || n.isChaiChainable(node)) {
        return;
      }
      increaseComplexity();
    },
