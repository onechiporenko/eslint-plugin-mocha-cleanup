...
function isChaiChainable(node) {
  return node.type === "MemberExpression" &&
    chainable.indexOf(node.property.name) !== -1;
}
...
function isBlockBody(node, type) {
  ...
  var parentType = obj.get(parent, "callee.object") ? "MemberExpression" : "Identifier";
...
function isTestSkipped(node) {
  ...
  if (callee.type === "Identifier") {
    return mocha.allSkipped.indexOf(callee.name) !== -1;
  }
  var o = callee.object.name;
