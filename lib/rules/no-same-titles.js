...
module.exports = {
  create (context) {
    ...
    const scope = hasOwnProperty.call(options, "scope") ? options.scope : "suite"
    ...
    return {
      ...
      CallExpression (node) {
        if (insideSuite && !(skipSkipped && n.tryDetectSkipInParent(node)) && mocha.tests.includes(obj.get(node, "callee.name"))) {
